        ...
        <section class="main">
            <div class="main-row">
                ...
                if (isset($_POST["btn-submit"])) {
                    ...
                    // echo  $title . $discription . $image . $created_at . $content;
                    // die;
                    $connection = new PDO("mysql:host=127.0.0.1;dbname=baileyshop;charset=utf8", "root", "");
                    $query = "UPDATE news SET title='$title',discription='$discription',image='$image',created_at='$created_at',content='$content' WHERE id =$id";
                    $stmt = $connection->prepare($query);
                    $stmt->execute();
                    ...
                    <div class="form">
                        <form action="" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form1">
                                <div class="form1-row">
                                    <span>Tiêu đề</span>
                                    <input type="text" onblur="checkTitle()" id="title" name="title" value="<?php echo $news['title'] ?>" autocomplete="off">
                                    <span class="error" id="errorTitle"></span>
                                </div>
                                <div class="form1-row">
                                    <span>Mô tả</span>
                                    <input type="text" onblur="checkDiscription()" id="dicription" name="discription" value="<?php echo $news['discription'] ?>" autocomplete="off">
                                    <span class="error" id="errorDiscription"></span>
                                </div>
                                <div class="form-row2a">
                                    <div class="form-row2a-col">
                                        <span>Hình ảnh</span>
                                        <input type="file" onblur="checkImg()" id="image" name="image" multiple value="<?php echo $news['image'] ?>">
                                        <span class="error" id="errorImg"></span>
                                    </div>
                                    <div class="form-row2a-col">
                                        <span>Ngày viết</span>
                                        <input type="date" onblur="checkDate()" id="date" name="created_at" value="<?php echo $news['created_at'] ?>">
                                        <span class="error" id="errorDate"></span>
                                    </div>
                                </div>
                                <div class="form2-row2b">
                                    <span>Nội dung</span>
                                    <input type="text" onblur="checkContent()" id="content" name="content" value="<?php echo $news['content'] ?>" autocomplete="off">
                                    <span class="error" id="errorContent"></span>
                                </div>
                                <div class="form1-btn">
                                    <button type="submit" class="btn btn-success" id="submit" name="btn-submit">Cập nhật</button>
                                </div>
                            </div>
                        </form>
                    </div>
